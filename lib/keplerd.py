...
class KeplerDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some keplerd methods
    ...
    @classmethod
    def from_kepler_conf(self, kepler_dot_conf):
        from kepler_config import KeplerConfig
        config_text = KeplerConfig.slurp_config_file(kepler_dot_conf)
        creds = KeplerConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = keplerlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = keplerlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = keplerlib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_keplerd_proto_version_with_sentinel_ping)
