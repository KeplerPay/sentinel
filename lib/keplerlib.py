...
def is_valid_kepler_address(address, network='mainnet'):
    ...
    try:
        ...
        # rescue from exception, not a valid Kepler address
        return False

    if (address_version != kepler_version):
        return False
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('kepler.conf')
...
def blocks_to_seconds(blocks):
    ...
    number of blocks.
    """
    return blocks * 2.3 * 60 # blocks every 2 mins but probably slower
