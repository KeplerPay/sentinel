...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, keplerd):
        golist = keplerd.rpc_command('gobject', 'list')
        ...
        for item in golist.values():
            try:
                (go, subobj) = self.import_gobject_from_keplerd(keplerd, item)
            except Exception as e:
    ...
    @classmethod
    def import_gobject_from_keplerd(self, keplerd, rec):
        import decimal
        import keplerlib
        import binascii
        import gobject_json
        ...
        # exclude any invalid model data from keplerd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from keplerd, with every run
        ...
        # get/create, then sync payment amounts, etc. from keplerd - Keplerd is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(keplerd)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from keplerd! %s" % e)
            govobj.vote_delete(keplerd)
            return (govobj, None)
    ...
    def vote_delete(self, keplerd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(keplerd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, keplerd, signal, outcome):
        import keplerlib
        ...
        output = keplerd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = keplerlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(keplerd, signal)

    def sync_network_vote(self, keplerd, signal):
        printdbg('\tSyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = keplerd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = KPLD_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 kepler addr, non-multisig
            if not keplerlib.is_valid_kepler_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Kepler address for network [%s], returning False" % (self.payment_address, config.network))
                return False
            ...
            # Kepler Core restricts proposals to 512 bytes max
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (keplerlib.blocks_to_seconds(superblockcycle) / 2) +
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = KPLD_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not keplerlib.is_valid_kepler_address(addr, config.network):
    ...
    def hash(self):
        import keplerlib
        return keplerlib.hashit(self.serialise())
